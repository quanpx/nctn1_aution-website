...
const YourAuctions = () => {
    ...
    if (data.length === 0) {
        return <h1>No auction available</h1>
    }
    ...
    return <div>
        {data.map(({ auction, created_at }, idx) => {
            console.log(auction);
            return <div key={idx} className="flex gap-x-10 mt-10 pb-2 border-b-4 border-black-500">
                ...
                <div className="pt-10 basis-1/2">
                    ...
                    <h1>Register at: {moment(created_at).format('LLLL')}</h1>
                    <h1>Time left: {differentTime(auction)}</h1>
                </div>
                <div className="p-5 pt-10 basis-1/4">
                    <Button type="primary" onClick={()=> navigate('/auction/'+auction.id)}>Go to auction</Button> <br/>
